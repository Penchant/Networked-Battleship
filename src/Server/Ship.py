from enum import Enum

class ShipOrientation(Enum):
    HorizontalRight = 0
    HorizontalLeft = 1
    VerticalUp = 2
    VerticalDown = 3

class ShipType(Enum):
    Carrier = ("C", 5)
    Battleship = ("B", 4)
    Cruiser = ("R", 3)
    Submarine = ("S", 3)
    Destroyer = ("D", 1)

    @classmethod
    def has_value(cls, value):
        return (any(value == item.value[0] for item in cls))

class Ship:
    # Initializes all properties
    def __new__(self, startX, startY, endX, endY, shipType):
        self.location = (startX, startY, endX, endY)
        self.shipType = shipType
        self.length = shipType.value[1]

        if(startX == endX):
            if(startX > endX):
                self.orientation = ShipOrientation.HorizontalLeft
            else:
                self.orientation = ShipOrientation.HorizontalRight
        else:
            if(startY > endY):
                self.orientation = ShipOrientation.VerticalDown
            else :
                self.orientation = ShipOrientation.VerticalUp
        return

    def __contains__(self, item):
        x = item[0]
        y = item[1]

        if(self.orientation == ShipOrientation.VerticalUp):
            if(x == self.location[0] & y >= self.location[1] & y <= self.location[1] + self.length):
                return True
            else: return False
        elif (self.orientation == ShipOrientation.VerticalDown):
            if (x == self.location[0] & y >= self.location[1] - self.length & y <= self.location[1]):
                return True
            else:
                return False
        elif (self.orientation == ShipOrientation.HorizontalRight):
                if (y == self.location[0] & x >= self.location[1] & x <= self.location[1] + self.length):
                    return True
                else:
                    return False
        else:
            if (y == self.location[0] & x >= self.location[1] - self.length & x <= self.location[1]):
                return True
            else:
                return False

    def validate(self):
        # if(self.location[0] != self.location[2] & self.location[1] != self.location[3]):
        #     print("Invalid " + self.shipType.name + ". Ship can not be diagonal\n")
        #     return False
        if (self.orientation == ShipOrientation.HorizontalRight | self.orientation == ShipOrientation.HorizontalLeft & abs(self.location[0] - self.location[2]) != self.shipType.value[1]):
            print("Invalid " + self.shipType.name + ". Ship has length " + abs(self.location[0] - self.location[2]) + " but should have length" + self.shipType[1] +" \n")
            return False
        else:
            return True

